    ...
    return WillPopScope(
      onWillPop: () async {
          ...
          if (presscount == 2) {
            ...
            var snackBar = const SnackBar(content: Text("กดอีกครั้งเพื่อออกจากแอพ"));
            ScaffoldMessenger.of(context).showSnackBar(snackBar)
              .closed.then((value) => presscount = 0);
            return false;
          }
        },
      child: (posts != null)
        ? Scaffold(
            ...
            floatingActionButton: SpeedDial(children: [
              SpeedDialChild(
                child: const Icon(Icons.add, color: Color(0xff795e35), size: 35),
                label: "สร้างโพสต์",
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const PostPage())),
              ),
              SpeedDialChild(
                child: const Icon(Icons.book, color: Color(0xff795e35), size: 35),
                label: "เพิ่มหนังสือใหม่",
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => AddBook(accType: "ADMIN"))),
              ),
...
class PostSec extends StatefulWidget {
  PostSec({super.key, required this.posts});
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, i) {
          return GestureDetector(
            onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => SocialPage(posts: widget.posts[i]))),
            child: SizedBox(
              height: 90,
              child: Card(
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      CircleAvatar(
                        backgroundImage: NetworkImage("${widget.posts[i]["Image"]}"),
