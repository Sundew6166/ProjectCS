...
class BottomBar extends StatefulWidget {
  BottomBar({super.key, required this.accType, required this.tab});
...
class _BottomBarState extends State<BottomBar> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Center(
        child: widget.accType == "USER" ? _userOptions.elementAt(currentIndex) : _adminOptions.elementAt(currentIndex)
      ),
      bottomNavigationBar: BottomNavigationBar(
        items: const [
          BottomNavigationBarItem(icon: Icon(Icons.home), label: "หน้าแรก"),
          BottomNavigationBarItem(icon: Icon(Icons.qr_code_scanner), label: "สแกน"),
