...
class _BuyPageState extends State<BuyPage> {
  ...
  Future<void> setData() async {
    if (widget.saleInfo["seller"] != FirebaseAuth.instance.currentUser!.uid) {
      await AccountController().getDeliveryInformation()
        .then((value) => setState(() => address = value["address"]));
      await BookController().checkHasBook(widget.saleInfo["book"]["isbn"], widget.saleInfo["book"]["edition"].toString())
        .then((value) => setState(() => canBuy = !value));
    }
  }
              ...
              Container(
                ...
                child: ElevatedButton(
                  onPressed: (canBuy)
                    ? () {
                        ...
                        ? showDialog(
                            ...
                            builder: (_) => AlertDialog(
                              title: const Text("ไม่สามารถสั่งซื้อได้"),
                              content: const Text("กรุณากรอกที่อยู่เพื่อจัดส่งก่อนทำรายการใหม่อีกครั้ง"),
                              actions: [
                                TextButton(
                                    ...
                                    try {
                                        ...
                                        .then((value) {
                                          ...
                                          if (statusBuy) {
                                            ...
                                            showDialog(
                                              context: context,
                                              builder: (_) => AlertDialog(
                                                title: const Text("ไม่สามารถซื้อได้ในขณะนี้"),
                                                content: const Text("มีคนอื่นกำลังซื้ออยู่"),
                                                actions: <Widget>[
                                                  TextButton(
                                      ...
                                      showDialog(
                                        ...
                                        builder: (_) => AlertDialog(
                                          ...
                                          content: const Text("เกิดข้อผิดพลาดในการซื้อหนังสือ กรุณาลองใหม่"),
                                          actions: <Widget>[
                                            TextButton(
                                              onPressed: () => Navigator.pop(context),
                                              child: const Text("ตกลง"),
                    ...
                    : null,
                  style: ElevatedButton.styleFrom(fixedSize: const Size(100, 40), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
                  child: const Text("ซื้อ", style: TextStyle(fontSize: 20))
                )
              ),
            ],
          )
        ...
        children: <Widget>[
          ...
          Expanded(
            child: Align(
              ...
              child: Text("$sellingPrice บาท", style: const TextStyle(fontSize: 16, color: Colors.red)),
            ),
          ),
        ],
      ),
