...
class SocialPage extends StatefulWidget {
  SocialPage({super.key, required this.posts});
...
class _SocialPageState extends State<SocialPage> {
  ...
  Future<void> setData() async {
    await PostController().getComment(widget.posts!["ID"])
    ...
    return Scaffold(
      ...
      body: (comments != null)
        ? RefreshIndicator(
            onRefresh: setData,
            child: CommentBox(
              userImage: CommentBox.commentImageParser(imageURLorPath: NetworkImage(user!.photoURL.toString())),
              ...
              sendButtonMethod: () async {
                if (commentController.text.isNotEmpty) {
                  try {
                    await PostController().addComment(commentController.text, widget.posts!["ID"])
                      .then((value) => Navigator.pushReplacement(
                        context,
                        PageRouteBuilder(
                          pageBuilder: (BuildContext context, Animation<double> animation1, Animation<double> animation2) {return super.widget;},
            ...
            child: Padding(
              ...
              child: Column(children: [
                Row(
                  children: [
                    CircleAvatar(
                      backgroundImage: NetworkImage("${widget.posts!["Image"]}"),
                    ...
                    Expanded(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Text("\t${widget.posts!["CreateBy"]}", overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 16)),
                          ]
                        )
                      ),
                    ),
                    Text("${widget.posts!["Create_DateTime_Post"]}", overflow: TextOverflow.ellipsis, textAlign: TextAlign.right),
                  ],
                ),
                Text("\t${widget.posts!["Detail_Post"]}", style: const TextStyle(fontSize: 16)),
              ])
            )
          )
        ),
            ...
            child: ListTile(
              leading: GestureDetector(
                child: Container(
                  ...
                  child: CircleAvatar(radius: 50, backgroundImage: CommentBox.commentImageParser(imageURLorPath: NetworkImage(data[i]["Image"]))),
                ),
              ),
