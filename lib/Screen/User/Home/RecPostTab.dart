...
class _RecPostTabState extends State<RecPostTab> {
  ...
  Future<void> setData() async {
    await PostController().getPostAll()
      .then((value) => setState(() => postList = value));
    await Recommendation().getRecommend()
      .then((value) => setState(() => recommendList = value));
  }
    ...
    return (postList != null && recommendList != null)
      ? Scaffold(
          ...
          floatingActionButton: FloatingActionButton(
            ...
            onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const PostPage())),
          ),
        )
      : Container(
...
class RecommendSection extends StatelessWidget {
  RecommendSection({super.key, required this.recommendList});
  List<dynamic> recommendList;
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Text("แนะนำสำหรับคุณ", style: TextStyle(fontSize: 20)),
        ...
        Container(
          ...
          child: ListView.builder(
            ...
            shrinkWrap: true,
            itemBuilder: (context, i) => RecommendCard(recommendList: recommendList[i])
          ),
        )
      ],
    );
...
class RecommendCard extends StatelessWidget {
  RecommendCard({super.key, required this.recommendList});
  dynamic recommendList;

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => ReviewPage(bookInfo: recommendList, hasBook: false, hasSale: false))),
      child: SizedBox(
        height: 200,
        width: 150,
        child: Card(
...
class PostSection extends StatefulWidget {
  PostSection({super.key, required this.postList});
  List<dynamic> postList;
      ...
      child: (widget.postList.isEmpty)
        ...
        : ListView.builder(
            ...
            itemBuilder: (context, i) {
              return GestureDetector(
                onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => SocialPage(posts: widget.postList[i]))),
                child: SizedBox(
                  height: 90,
                  child: Card(
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          CircleAvatar(
                            backgroundImage: NetworkImage("${widget.postList[i]["Image"]}"),
