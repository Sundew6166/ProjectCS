...
class PostController {
  Future<List<dynamic>> getPostAll() async {
    ...
    await db.collection("posts")
      .orderBy('Create_DateTime_Post', descending: true).get()
  ...
  Future<List<dynamic>> getMyPost() async {
    ...
    await db.collection('posts').where('CreateBy', isEqualTo: user!.uid).get().then(
      (value) {
    ...
    output.sort(
      (a, b) => b['Create_DateTime_Post'].compareTo(a['Create_DateTime_Post'])
    );
    return output;
  }
  ...
  Future<List<dynamic>> getComment(String idpost) async {
    ...
    await db.collection("comments").where('ID_Post', isEqualTo: idpost).get().then(
      (querySnapshot) async {
        for (var docSnap in querySnapshot.docs) {
          Map<String, dynamic> acc = await AccountController()
            .getAnotherProfile(docSnap.data()['CreateBy']);
