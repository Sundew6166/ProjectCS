...
class BookController {
  ...
  void linkTypeAndBook(String bookId, List<String> types) async {
    ...
    for (var type in types) {
      print("type: " + type);
      await db.collection('book_types').where("name", isEqualTo: type).get().then(
        (value) {
          if (value.docs.isNotEmpty) {
            print("add b_has_bt");
            db.collection('b_has_bt').add({"book": bookId, "type": value.docs[0].id});
          } else {
  ...
  Future<Map<String, dynamic>?> getBookInfo(String id) async {
    final db = FirebaseFirestore.instance;
    Map<String, dynamic>? bookInfo;

    await db.collection('books').doc(id).get().then(
      (value) => bookInfo = value.data()
    );
    await getTypesOfBook(id).then(
      (value) => bookInfo!.addAll({"types": value})
    );
    return bookInfo;
  }
  ...
  Future<List<String>> getTypesOfBook(String idBook) async {
    ...
    await db.collection('b_has_bt').where("book", isEqualTo: idBook).get().then(
      (querySnapshot) async {
        for (var docSnap in querySnapshot.docs) {
          await db.collection('book_types').doc(docSnap.data()['type']).get().then(
            (value) => types.add(value.data()!['name'])
          );
        }
      }
    );
  ...
  Future<void> deleteBookFromLibrary(String isbn, String edition) async {
    ...
    await db
        ...
        .get()
        .then((value) => db.collection('a_has_b').doc(value.docs[0].id).delete());
  }
  ...
  Future<List<String>> getEditionsBook(String isbn) async {
    ...
    await db.collection('books').where('isbn', isEqualTo: isbn).get().then(
      (querySnapshot) {
        for (var docSnap in querySnapshot.docs) {
  ...
  Future<bool> checkHasBook(String isbn, String edition) async {
    ...
    await db.collection('a_has_b')
      .where('account', isEqualTo: user!.uid)
      .where('book', isEqualTo: '${isbn}_${edition}').get().then(
        (value) {
  ...
  Future<void> updateBookInfo(String idBook, String title, String author, String publisher, int price, List<String> oldTypes, List<String> newTypes, String synopsys, String oldCoverImage, File? newCoverImage, String createBy) async {
    ...
    oldTypes.forEach((element) {
      if (newTypes.contains(element)) {toRemove.add(element);}
    });
      ...
      await db.collection('book_types').where('name', isEqualTo: type).get().then(
        (docType) {
          db.collection('b_has_bt')
            ...
            .where('type', isEqualTo: docType.docs[0].id).get().then(
              (value) => db.collection('b_has_bt').doc(value.docs[0].id).delete()
            );
        }
      );
    }
  ...
  Future<List<Map<String, dynamic>?>> getAllBookInLibrary(String accType) async {
    ...
    if (accType == "ADMIN") {
      await db.collection('books').where('approveStatus', isEqualTo: true).get()
      ...
      final user = FirebaseAuth.instance.currentUser;
      await db.collection('a_has_b').where('account', isEqualTo: user!.uid).get()
        .then((querySnapshot) async {
          for (var docSnap in querySnapshot.docs) {
            var idSplit = docSnap.data()['book'].split("_");
            if (!await SaleController().checkHasSale(idSplit[0], idSplit[1])) {
              await db.collection('books').doc(docSnap.data()['book']).get().then(
                (value) async {
                  var data = value.data();
  ...
  Future<List<Map<String, dynamic>?>> getAllBookPendingApproval() async {
    final db = FirebaseFirestore.instance;
    List<Map<String, dynamic>?> output = [];
    await db.collection('books').where('approveStatus', isEqualTo: false).get().then(
      (querySnapshot) {
        for (var docSnap in querySnapshot.docs) {
  ...
  Future<List<Map<String, dynamic>?>> getAllMyBook() async {
    ...
    final user = FirebaseAuth.instance.currentUser;
    List<Map<String, dynamic>?> output = [];
    await db.collection('a_has_b').where('account', isEqualTo: user!.uid).get().then(
      (querySnapshot) async {
        for (var docSnap in querySnapshot.docs) {
          await db.collection('books').doc(docSnap.data()['book']).get().then(
            (value) async {
              var data = value.data();
              data!['createDateTime'] = docSnap.data()['createDateTime'];
              await getTypesOfBook(docSnap.id).then(
                (value) => data.addAll({"types": value})
              );
              output.add(data);
            }
          );
